      ...
      <View style={styles.fixedTabContainer}>
        <TouchableOpacity style={styles.tabButton} onPress={() => navigation.navigate('ToPay', { user })}>
          <View style={styles.tabInner}>
            ...
            {counts.toPay > 0 && (
              <View style={styles.badge}><Text style={styles.badgeText}>{counts.toPay}</Text></View>
            )}
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.tabButton, styles.activeTabButton]}>
          <View style={styles.tabInner}>
            ...
            {counts.toConfirm > 0 && (
              <View style={styles.badge}><Text style={styles.badgeText}>{counts.toConfirm}</Text></View>
            )}
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.tabButton} onPress={() => navigation.navigate('ToReceive', { user })}>
          <View style={styles.tabInner}>
            ...
            {counts.toReceive > 0 && (
              <View style={styles.badge}><Text style={styles.badgeText}>{counts.toReceive}</Text></View>
            )}
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.tabButton} onPress={() => navigation.navigate('ToRate', { user })}>
          <View style={styles.tabInner}>
            ...
            {counts.toRate > 0 && (
              <View style={styles.badge}><Text style={styles.badgeText}>{counts.toRate}</Text></View>
            )}
          </View>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: 15, paddingVertical: 12, backgroundColor: colors.white,
  ...
  tabButton: { justifyContent: 'center', alignItems: 'center', paddingHorizontal: 8 },
  tabInner: { flexDirection: 'row', alignItems: 'center' },
  tabText: { fontSize: 14, color: colors.textSecondary, fontWeight: '500' },
  activeTabButton: {},
  activeTabText: {
    fontWeight: 'bold', color: colors.primaryGreen,
    borderBottomWidth: 2, borderBottomColor: colors.primaryGreen, paddingBottom: 2,
  },
  badge: {
    backgroundColor: colors.red, borderRadius: 10, minWidth: 18, height: 18,
    justifyContent: 'center', alignItems: 'center', marginLeft: 5, paddingHorizontal: 4,
  },
  badgeText: { color: colors.white, fontSize: 10, fontWeight: 'bold' },

  summaryContainer: {
    backgroundColor: colors.white, marginHorizontal: 15, marginTop: 10, padding: 15,
  ...
  orderCard: {
    backgroundColor: colors.white, marginTop: 10, marginHorizontal: 15, padding: 15,
    borderRadius: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.1, shadowRadius: 2, elevation: 2,
  },
  ...
  itemRow: {
    ...
    paddingBottom: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.greyBorder,
  },
  ...
  bottomNav: {
    position: 'absolute', bottom: 0, width: '100%', backgroundColor: colors.primaryGreen,
